...
class Workload:
    ...
    def gen_database(self):
        ...
        for i in range(1, self.seller_num + 1):
            ...
            for j in range(1, self.store_num_per_user + 1):
                ...
                assert code == 200, f"Failed to create store {store_id} for seller {user_id}"
    ...
    def get_new_order(self) -> NewOrder:
        ...
        for store_id in self.store_ids:
            print(f"Store ID: {store_id}, Available Books Count: {len(self.book_ids[store_id])}")
        ...
        if len(self.book_ids[store_id]) == 0:
            print(f"No books available for Store ID: {store_id}. Skipping this order.")
